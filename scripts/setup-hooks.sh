...
# (GitHub-Common); a consuming repo's thin setup-hooks.sh exports
# GHCOMMON_TARGET_REPO so the same one-time wiring applies to THAT repo
...
repo_root="${GHCOMMON_TARGET_REPO:-$(cd "${script_dir}/.." && pwd)}"
