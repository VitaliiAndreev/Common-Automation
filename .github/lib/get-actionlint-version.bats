#!/usr/bin/env bats
# Unit tests for get-actionlint-version.sh - the single accessor for
# the actionlint version. Mirrors get-bats-version.bats: two branches,
# an override argument echoed verbatim and the default path that reads
# ACTIONLINT_VERSION from the adjacent versions.env, plus a guard for
# the unset case so a missing pin fails loudly rather than silently
# producing an empty version string.
# Run with: bats lib/get-actionlint-version.bats

SCRIPT="${BATS_TEST_DIRNAME}/get-actionlint-version.sh"
...
@test "exits non-zero when ACTIONLINT_VERSION is unset" {
    # Stage a versions.env without ACTIONLINT_VERSION in a temp dir and
    ...
    tmp="$(mktemp -d)"
    cp "${SCRIPT}" "${tmp}/get-actionlint-version.sh"
    printf 'BATS_VERSION=1.11.0\n' > "${tmp}/versions.env"
    run "${tmp}/get-actionlint-version.sh"
    rm -rf "${tmp}"
    [ "${status}" -ne 0 ]
}
