#!/usr/bin/env bash
# Prints the actionlint version to use: an explicit override argument
# wins, otherwise the canonical ACTIONLINT_VERSION from the adjacent
# versions.env. Single accessor for the actionlint version - the
...
# place.
#
# Usage: get-actionlint-version.sh [override]
...
# set -u makes a missing ACTIONLINT_VERSION fail loudly rather than
